...
// KNOWN DIVERGENCE (2026-08-18). The pairs below are the human-verified answers.
// The deployed contract's cached verdicts currently disagree on seven stages:
//
//   stage      here (correct)     cached on-chain verdict
//    4         Pink + White       Purple + White
//    5         Red + Orange       Red + Brown
//    9         Pink + Red         White + Pink
//   10         White + Cyan       Cyan + Blue
//   12         Indigo + Silver    Silver + Purple
//   13         Pink + Cyan        Pink + White
//   19         Purple + Silver    Silver + Indigo
//
// The validators over-weight the visor and the black limbs — stage 4 reads as
// 40% purple to them, where a pixel count over the opaque subject gives
// White 55 / Pink 34 / Purple 8.
//
// This matters because `submit_pick` scores on `set(picks) == set(final_colors)`
// and nothing else. Until those seven are re-judged with `analyze_stage`, an
// online round scores them by the verdicts above and an offline round scores
// them by this file, so the same picks can be right in one and wrong in the
// other. Re-run `analyze_stage` (owner-only) for those ids and re-check; the
// judgment is nondeterministic, so a re-roll may need more than one attempt.
